...
import { CommunityCTA, IncomingBooks} from "./Sidebar"
...
export function BookReaderApp({ className, ...props }: React.ComponentProps<"div">) {
  ...
  // Only show search results after the user explicitly submits (Enter or category select)
  const isSearching = submittedQuery.trim().length > 0 || selectedCategoryId !== null
  ...
  const handleCategorySelect = (id: number | null) => {
    ...
    search({
      ...
      category: id !== null ? String(id) : undefined,
    })
  }

  // Default view: group all books
        ...
        {isSearching ? (
          <div className="flex flex-col gap-2">
            <p className="text-xs text-muted-foreground px-1">
              {searchLoading
                ? "Searching…"
                : `${searchResults.length} result${searchResults.length !== 1 ? "s" : ""}${submittedQuery.trim() ? " for " : ""}`}
              {!searchLoading && submittedQuery.trim() && (
            ...
            {!searchLoading && searchResults.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground">
                ...
                <p className="text-xs">
                  {submittedQuery.trim()
                    ? `No books found for "${submittedQuery}"${selectedCategoryId ? ` in "${selectedCategory?.name}"` : ""}`
